...
func (s SqlTokenStore) Save(token *model.Token) StoreChannel {
	...
	go func() {
		...
		if err := s.GetMaster().Insert(token); err != nil {
			result.Err = utils.NewLocAppError("SqlTokenStore.Save", "store.sql_recover.save.app_error", nil, "")
		}
...
func (s SqlTokenStore) Delete(token string) StoreChannel {
	...
	go func() {
		...
		if _, err := s.GetMaster().Exec("DELETE FROM Tokens WHERE Token = :Token", map[string]interface{}{"Token": token}); err != nil {
			result.Err = utils.NewLocAppError("SqlTokenStore.Delete", "store.sql_recover.delete.app_error", nil, "")
		}
	...
	go func() {
		...
		if err := s.GetReplica().SelectOne(&token, "SELECT * FROM Tokens WHERE Token = :Token", map[string]interface{}{"Token": tokenString}); err != nil {
			if err == sql.ErrNoRows {
				result.Err = utils.NewAppError("SqlTokenStore.GetByToken", "store.sql_recover.get_by_code.app_error", nil, err.Error(), http.StatusBadRequest)
			} else {
				result.Err = utils.NewAppError("SqlTokenStore.GetByToken", "store.sql_recover.get_by_code.app_error", nil, err.Error(), http.StatusInternalServerError)
			}
		}
...
func (s SqlTokenStore) Cleanup() {
	...
	deltime := utils.GetMillis() - model.MAX_TOKEN_EXIPRY_TIME
