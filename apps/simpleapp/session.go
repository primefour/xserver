...
func GetSession(token string) (*model.Session, *utils.AppError) {
	...
	if session == nil {
		if sessionResult := <-Srv.Store.Session().Get(token); sessionResult.Err != nil {
			return nil, utils.NewLocAppError("GetSession", "api.context.invalid_token.error", map[string]interface{}{"Token": token, "Error": sessionResult.Err.DetailedError}, "")
		} else {
			session = sessionResult.Data.(*model.Session)

			if session == nil || session.IsExpired() || session.Token != token {
				return nil, utils.NewLocAppError("GetSession", "api.context.invalid_token.error", map[string]interface{}{"Token": token, "Error": ""}, "")
			} else {
				AddSessionToCache(session)
				return session, nil
			}
	...
	if session == nil || session.IsExpired() {
		return nil, utils.NewLocAppError("GetSession", "api.context.invalid_token.error", map[string]interface{}{"Token": token}, "")
	}

	return session, nil
}

func GetSessions(userId string) ([]*model.Session, *utils.AppError) {
...
func RevokeAllSessions(userId string) *utils.AppError {
	if result := <-Srv.Store.Session().GetSessions(userId); result.Err != nil {
		return result.Err
	} else {
...
func AddSessionToCache(session *model.Session) {
	sessionCache.AddWithExpiresInSecs(session.Token, session, int64(*SServConfig.ServiceSettings.SessionCacheInMinutes*60))
}
...
func RevokeSessionsForDeviceId(userId string, deviceId string, currentSessionId string) *utils.AppError {
	if result := <-Srv.Store.Session().GetSessions(userId); result.Err != nil {
		return result.Err
	} else {
		sessions := result.Data.([]*model.Session)
...
func RevokeSessionById(sessionId string) *utils.AppError {
...
func RevokeSession(session *model.Session) *utils.AppError {
	if session.IsOAuth {
	} else {
		if result := <-Srv.Store.Session().Remove(session.Id); result.Err != nil {
			return result.Err
...
func AttachDeviceId(sessionId string, deviceId string, expiresAt int64) *utils.AppError {
